		...
		for (var j = 0; j < board[i].length; j++) {
			switch(gamemode) {
				...
				case 2:
					...
					switch(i) {
						case board.length-1:
							if (boardTemplate[2][0][j] != 0) {
								board[i][j] = [boardTemplate[2][0][j],0]
							}
						break;
					}
				break;
...
function getPossibleMoves(index) {
	...
	switch(pieceType[0]) {
		...
		case 8:
			//trill
			possibleMoves = trill(possibleMoves, y, x, colour, board, false)
		break;
	}
...
// probably a more artistic method possible
function trill(possibleMoves, y, x, colour, board, checkCall) {
	var foundCheck = false
	for (var i = -2; i <= 2; i++) {
		for (var j = -2; j <= 2; j++) {
			if (i != 0 || j != 0) {
				if(y+i>=0&&y+i<board.length&&x+j>=0&&x+j<board[y].length) {
					...
					if(i == 0) {
						possibleMoves.push([y,x+j])
					}
					if(Math.abs(i) == 2 && Math.abs(j) == 2) {
						possibleMoves.push([y+i,x+j])
					}
					if(i==2&&j==-1){possibleMoves.push([y+i,x+j])}
					if(i==1&&j==2){possibleMoves.push([y+i,x+j])}
					if(i==-2&&j==1){possibleMoves.push([y+i,x+j])}
					if(i==-1&&j==-2){possibleMoves.push([y+i,x+j])}
				}
			}
